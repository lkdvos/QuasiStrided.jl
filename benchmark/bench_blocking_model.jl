...
# over, per real dtype:
#
#   * named points: the analytical model (`_modelled_blocking`), the fallback
#     row (`_fallback_blocking`) and the previous fallback (`oldfallback`, the
#     pre-2026-09-25 constants), plus the model with K split into equal blocks
#     per shape (`model_bal`), also on `KSHAPES`, whose K extents are not
#     powers of two (measured, no effect -- see `_modelled_blocking`);
...
# `Qk` split into the fewest blocks of at most `kc`, equal to within one.
balanced_kc(kc::Int, Qk::Int) = cld(Qk, cld(Qk, kc))
...
# The named rows as (set, combo) at one shape, plus the model row with K
...
function named_points(rows, spec)
    ...
    v = rows.model
    v === nothing || push!(pts, ("model_bal", (v.mc, balanced_kc(v.kc, spec.Ka), v.nc)))
    return pts
end
...
const NAMED_SETS = ("model", "model_bal", "fallback", "oldfallback")
...
function summarize(io, raw)
    ...
    for T in DTYPES
        ...
        @printf(
            io, "orig 36-grid, MAIN_SHAPES, SIMD only: best %.4f %s  worst %.4f %s  spread %.1f%%\n",
            o[1][2], o[1][1], o[end][2], o[end][1], 100 * (o[end][2] / o[1][2] - 1)
        )
        if WITH_SCALAR
            os = rank_sets(raw, T, ("orig",), main_names; kernels = ("SIMDKernel", "ScalarKernel"))
            @printf(
                io, "orig 36-grid, MAIN_SHAPES, SIMD+Scalar jointly (the 9%%/11%% method): spread %.1f%%\n",
                100 * (os[end][2] / os[1][2] - 1)
            )
        end
        w = rank_sets(raw, T, ("wide", "orig"), grid_names)
        @printf(
            io, "grid points, GRID_SHAPES: best %.4f %s  worst %.4f %s  spread %.1f%%\n",
